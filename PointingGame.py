    ...
    # --- FINISHED ---
        ...
        if final_submit:
            if st.session_state.all_results:
                ...
                st.markdown("---")
                st.error("もし上のボタンでダウンロードできない場合（LINEなどで開いている場合）")
                st.write("以下のテキストボックスの中身を **「すべて選択」してコピー** し、LINEなどで実験者に直接送ってください。")
                
                # データフレームをCSVテキストとして表示（コピー用）
                csv_text = df.to_csv(index=False)
                st.text_area("📋 コピー用データエリア", value=csv_text, height=200)
